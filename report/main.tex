...
\begin{abstract}
We present a reinforcement learning framework for autonomous 3D scene reconstruction, in which a robot learns to plan camera trajectories that maximize reconstruction quality within a fixed time budget.
...
Depth observations are incrementally fused into a Truncated Signed Distance Field (TSDF) using either Open3D or NVIDIA nvblox, and the resulting reconstruction is compared against a ground-truth mesh at every step to compute the reward signal.
We formulate the problem as a Markov Decision Process and train the policy with Proximal Policy Optimization (PPO), comparing two reconstruction error metrics---Chamfer distance and a voxel-wise TSDF error---as well as two reward shaping strategies.
The policy network combines a Transformer-based camera pose history encoder, CNN-based image feature extractors, and a 3D-CNN weight-grid encoder via a modular feature extraction architecture.
Experiments show that the learned policy consistently improves reconstruction quality over the course of each episode and outperforms an untrained random baseline, while a hand-crafted scripted policy that traverses the table edges serves as an additional reference.
\end{abstract}
...
In this work, we develop an end-to-end RL framework in which a simulated Franka Panda manipulator learns to move its wrist-mounted camera so as to maximize the reconstruction quality of a tabletop scene within a fixed episode horizon.
Our contributions are as follows:
\begin{itemize}
    \item A modular simulation environment built on robosuite~\cite{robosuite2020} and Gymnasium that couples physics-based robot control with incremental TSDF reconstruction.
    \item A comparison of two volumetric reconstruction back-ends (Open3D~\cite{zhou2018open3d} and NVIDIA nvblox) and two error metrics (Chamfer distance and voxel-wise TSDF error).
    \item A multi-modal policy architecture that fuses camera pose history (via a Transformer encoder), image observations (via CNNs), and volumetric observation weights (via a 3D-CNN).
    \item Empirical results demonstrating learned exploration strategies that steadily improve reconstruction quality during each episode.
\end{itemize}
...
Classical NBV methods greedily select the next viewpoint that maximizes an information-theoretic criterion.
Meli and Dehio~\cite{meli2025robotcellmodelingexploratory} recently explored RL-based exploratory robot motions for environment modeling.
Our work follows a similar motivation but focuses on the interplay between the choice of reconstruction metric and reward shaping for effective policy learning.

\textbf{Reinforcement Learning for Robot Control.}\quad
...
The robosuite simulator~\cite{robosuite2020} provides standardized manipulation environments with MuJoCo physics and has been extended in this project with a custom reconstruction task.
...
% \centering
% \begin{tikzpicture}[
%   box/.style={draw, thick, rounded corners=3pt, minimum width=32mm, minimum height=12mm, align=center, font=\small},
%   arrow/.style={-{Latex[length=2.5mm]}, thick},
%   dashedarrow/.style={-{Latex[length=2.5mm]}, thick, dashed},
%   lbl/.style={font=\scriptsize\sffamily},
% ]
% \node[box] (robot) {Robot Policy\\(RL Agent)};
% \node[box, below left=10mm and 14mm of robot] (env) {Robot Environment\\{\scriptsize\textit{(Reconstruct3D)}}};
% \node[box, below right=10mm and 14mm of robot] (recon) {Reconstruction\\Policy};

% \node[draw=black!60, dashed, rounded corners=5pt, fit=(env)(recon), inner sep=5mm] (group) {};
% \node[below=1mm of group, lbl, align=center, text width=70mm] {RL Environment {\scriptsize\textit{(Gymnasium wrapper)}}};

% \draw[arrow] (robot.west) -| node[lbl, near start, above] {action} (env.north);
% \draw[arrow] (env.north east) |- node[lbl, near end, above] {reward} (robot.south west);
% \draw[arrow] (env.south east) |- node[lbl, near end, above, text width=25mm, align=center] {RGB-D + pose} (recon.south west);
% \draw[arrow] (recon.north west) |- node[lbl, near end, above, text width=22mm, align=center] {mesh / TSDF} (env.north east);
% \draw[dashedarrow] (env.north west) |- node[lbl, near end, above] {pose, images} (robot.north west);
% \draw[dashedarrow] (recon.north east) |- node[lbl, near end, above, text width=22mm, align=center] {reconstruction obs} (robot.north east);
% \end{tikzpicture}
% \caption{System architecture. Solid arrows show the simulation loop; dashed arrows show observation flow to the RL agent. The wrapper encapsulates the environment and reconstruction module behind a standard Gymnasium interface.}
% \label{fig:architecture2}
% \end{figure*}
