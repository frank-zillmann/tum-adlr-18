...
\caption{Screenshot of a evaluation episode.}
...
\begin{itemize}
    ...
    \item \textbf{Robot Policy:} The reinforcement learning policy and its neural architecture that decides the robot's actions based on its several observations.
\end{itemize}
...
Images are resized to $64 \times 64$ in the forward pass. Furthermore, the use can choose to only use the RGB image, only the mesh rendering, neither, or both as input to the policy.
...
Training uses PPO mostly with default parameters. The discount factor $\gamma$ was set to 0.98 to because of the rather short horizon and the entropy coefficient was set to 0.01 to encourage exploration.
...
Training was conducted on a CPU-only laptop (Open3D, Chamfer distance metric) and on a Google Cloud VM with an NVIDIA T4 GPU (nvblox back-end, voxel-wise TSDF Error) and many of the discussed features where implemented in an iterative manner depending on previous results over the course of the project, so a direct comparison of the training curves is not possible.
...
A large problem for the effective training and evaluation of the described approaches were memory leaks in the used libraries, which prevented long training runs.
While a initial Segmentation Fault caused by the rendering of the current reconstruction could be fixed by switching from Open3D's visualizer to a custom off-screen renderer, a leak in nvblox caused the GPU memory usage to grow over time until the process was killed some hours of training.
...
First run showed the robot strongly jiggling the camera and frequently moving it away from the scene, which could be mostly fixed through the addition of the action penalty and the delta-based reward shaping.
...
Adding the camera pose history via the Transformer encoder improved performance, which is expected as the agent can better avoid revisiting the same viewpoints but it might also have lead to less dependence on the configuration of the objects on the table as the policy can learn to just follow a fixed sweeping trajectory across the table without needing to adapt to the specific scene.
Further ablation studies would be needed to investigate this hypothesis.

The pre-final run \ref{fig:training_curve} showed a good increase in evaluation reward and even slightly beats the scripted baseline which moves over the table in a fixed trajectory and yields a average reward of 19.86 ± 4.3.
This policy often steered to robot to the side and below the table at the end of the episode, which is likely due to the fact that there are voxels below the table which can sometimes be observed through this or because the reward signal becomes very noisy when the reconstruction is already good.
To tackle further, the below-table voxels were excluded from the reward computation. The final training run with this change and the addition of the birdview image as input does not show this behavior anymore and confirms the hypothesis.
