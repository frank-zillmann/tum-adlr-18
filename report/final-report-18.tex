...
The robot carries an RGB-D camera with ($128 \times 128$\,pixels) on its end-effector (TCP).
...
It only allows to query the reconstructed mesh, so the reward is computed via Chamfer distance to the ground-truth mesh.

\textbf{NVIDIA nvblox} TODO cite: Provides GPU-accelerated TSDF integration in a similar manner to Open3D but requires an NVIDIA GPU and CUDA drivers.
...
Both back-ends share a common interface (\texttt{BaseReconstructionPolicy}) with \texttt{add\_obs}, \texttt{reconstruct}, and \texttt{reset} methods, making it easy to switch between through the config file without changing the training code and to potentially add new reconstruction methods.
...
TODO ref to result, Voxels lower that $z_\text{table\_surface} - 0.5 \cdot d_\text{trunc}$ are excluded to avoid penalizing physically inaccessible regions.
...
\end{itemize}
An classic action penalty $\lambda \cdot \overline{|\mathbf{a}_t|}$ with $\lambda = 0.1$ discourages unnecessarily large motions.
